...
export function RescheduleBookingDialog({
  ...
  const [selectedNewDate, setSelectedNewDate] = useState<Date | undefined>(() => {
    try {
      return parseISO(booking.date); // Initialize with current booking date
    } catch (e) {
      console.error("Error parsing initial booking date for reschedule dialog:", booking.date, e);
          ...
          <div>
            <Card className="mb-6">
              ...
              <CardContent className="space-y-1 text-sm">
                ...
                <p className="flex items-center"><Calendar className="w-4 h-4 mr-2 text-muted-foreground"/><strong>Date:</strong> {format(parseISO(booking.date), 'EEEE, MMM d, yyyy')}</p>
