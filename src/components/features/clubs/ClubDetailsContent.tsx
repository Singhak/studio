...
export function ClubDetailsContent() {
  ...
  const clubIdParam = Array.isArray(params.clubId) ? params.clubId[0] : params.clubId;
  ...
  useEffect(() => {
    ...
    const fetchClubDetails = async () => {
      ...
      setClubError(false);
      try {
        const fetchedClub = await getClubById(clubIdParam);
        if (fetchedClub) {
          setClub(fetchedClub);
        } else {
          setClubError(true);
        ...
        console.error(`Failed to fetch club details for ${clubIdParam}:`, err);
        setClubError(true);
      } finally {
        setIsLoadingClub(false);
      }
  ...
  useEffect(() => {
    const loadServicesForCurrentClub = async () => {
      ...
      if (!club?._id) {
        if (!isLoadingClub) setServicesError("Club information is not available.");
      ...
      try {
        ...
        console.error(`[ClubDetailsContent] Failed to fetch services for club ${club._id}:`, error);
  ...
  const handleBookSlot = async () => {
    if (!currentUser) {
      toast({
        ...
        toastDescription: "Please log in or register to continue.",
        toastAction: (
    ...
    if (!selectedServiceForBooking || !selectedBookingDate || !selectedBookingSlot) {
      toast({ variant: "destructive", toastTitle: "Incomplete Selection", toastDescription: "Please select a service, date, and time slot." });
      return;
    }

    setIsBooking(true);
    toast({ toastTitle: "Submitting Booking Request..." });
    ...
    try {
      ...
      toast({ toastTitle: "Booking Submitted!", toastDescription: bookingResponse.message });
      addNotification(`New Booking Request: ${club?.name}`, `A new booking for "${selectedServiceForBooking.name}" has been requested.`, '/dashboard/owner');
      addNotification(`Booking for ${selectedServiceForBooking.name} Pending`, `Your booking request for ${club?.name} is ${bookingResponse.status}.`, '/dashboard/user');
      setSelectedBookingSlot(null);
    } catch (error) {
      toast({ variant: "destructive", toastTitle: "Booking Failed", toastDescription: error instanceof Error ? error.message : "An unexpected error occurred." });
    } finally {
      setIsBooking(false);
    }
  };
  ...
  const availableSportTypes = useMemo(() => {
    ...
    return Array.from(new Set<SportType>(fetchedServices.map(service => service.sportType)));
  }, [fetchedServices]);
  ...
  if (clubError || !club) {
    return (
      <div className="text-center py-12">
        ...
        <Button asChild className="mt-4"><a href="/clubs">Back to Clubs</a></Button>
      </div>
    );
  }
    ...
    <>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <Card><CardHeader><CardTitle className="text-2xl">About {club.name}</CardTitle></CardHeader><CardContent><p className="text-muted-foreground leading-relaxed">{club.description}</p>{club.amenities && club.amenities.length > 0 && (<div className="mt-4"><h3 className="text-md font-semibold mb-2 text-foreground">Amenities:</h3><div className="flex flex-wrap gap-2">{club.amenities.map(amenity => (<Badge key={amenity} variant="outline" className="flex items-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5 text-primary"/> {amenity}</Badge>))}</div></div>)}</CardContent></Card>
          <Card>
            <CardHeader className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2"><CardTitle className="text-2xl">Services & Pricing</CardTitle>{availableSportTypes.length > 1 && (<div className="w-full sm:w-auto sm:min-w-[200px]"><Select value={selectedSportFilter} onValueChange={(value) => setSelectedSportFilter(value as SportType | "all")}><SelectTrigger className="w-full" aria-label="Filter services by sport type"><Filter className="h-4 w-4 mr-2 text-muted-foreground" /><SelectValue placeholder="Filter by sport..." /></SelectTrigger><SelectContent><SelectItem value="all">All Sports</SelectItem>{availableSportTypes.map(sport => (<SelectItem key={sport} value={sport}>{sport}</SelectItem>))}</SelectContent></Select></div>)}</CardHeader>
            <CardContent>
        ...
        <div className="space-y-8">
          ...
          <Button size="lg" className="w-full text-lg py-6" onClick={handleBookSlot} disabled={!selectedServiceForBooking || !selectedBookingDate || !selectedBookingSlot || isBooking || isLoadingServices || !!servicesError}>{isBooking ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <CreditCard className="mr-2 h-5 w-5" />} Book Selected Slot</Button>
          <Card><CardHeader><CardTitle className="text-xl">Contact Information</CardTitle></CardHeader><CardContent className="space-y-3">{club.contactPhone && (<div className="flex items-center"><Phone className="w-4 h-4 mr-2 text-primary" /><a href={`tel:${club.contactPhone}`} className="text-muted-foreground hover:text-primary">{club.contactPhone}</a></div>)}{club.contactEmail && (<div className="flex items-center"><Mail className="w-4 h-4 mr-2 text-primary" /><a href={`mailto:${club.contactEmail}`} className="text-muted-foreground hover:text-primary">{club.contactEmail}</a></div>)}{club.address?.street && club.address?.city && (<div className="flex items-center"><MapPin className="w-4 h-4 mr-2 text-primary" /><span className="text-muted-foreground">{`${club.address.street}, ${club.address.city}`}</span></div>)}</CardContent></Card>
          <Card>
